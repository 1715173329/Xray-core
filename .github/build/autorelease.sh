...
# Make sure we could caught err code
set -e
...
readonly release_ver="${RELEASE_TAG#*v}"
...
-X ${go_pkg}/core.build=Release
-X ${go_pkg}/core.version=${release_ver}
-buildid="

#  os      arch      arm  mips       mips64     snap
readonly platforms="\
android    arm64     0    0          0          aarch64
darwin     amd64     0    0          0          0
darwin     arm64     0    0          0          aarch64
dragonfly  amd64     0    0          0          0
freebsd    386       0    0          0          0
freebsd    amd64     0    0          0          0
freebsd    arm       7    0          0          armv7
freebsd    arm64     0    0          0          aarch64
linux      386       0    0          0          0
linux      amd64     0    0          0          amd64
linux      arm       5    0          0          armv5
linux      arm       6    0          0          armv6
linux      arm       7    0          0          armv7
linux      arm64     0    0          0          aarch64
linux      mips      0    hardfloat  0          0
linux      mips      0    softfloat  0          0
linux      mips64    0    0          hardfloat  0
linux      mips64    0    0          softfloat  0
linux      mips64le  0    0          hardfloat  0
linux      mips64le  0    0          softfloat  0
linux      mipsle    0    hardfloat  0          0
linux      mipsle    0    softfloat  0          0
linux      ppc64     0    0          0          0
linux      ppc64le   0    0          0          0
linux      riscv64   0    0          0          0
linux      s390x     0    0          0          0
openbsd    arm       7    0          0          armv7
openbsd    arm64     0    0          0          aarch64
windows    386       0    0          0          x86
windows    amd64     0    0          0          x86-64
windows    arm       7    0          0          armv7"
...
function pack_package(){
	case "$1" in
	"windows")
		zip -qr "${package_name}" "${go_build_dir}"/* "${data_dir}"/*
		;;
	*)
		tar -zcf "${package_name}" --transform 's|.*/||g' "${go_build_dir}"/* "${data_dir}"/* \
			2>"/dev/null"
		;;
	esac
...
echo "$platforms" | while read -r os arch arm mips mips64 snap
do
	echo -e "[Building] GOOS: $os GOARCH: $arch GOARM: $arm GOMIPS: $mips GOMIPS64: $mips64 SNAP: $snap"
	for type in {"build","pack"}
	do
		case "$arch" in
		"arm")
			go_build_dir="build_${os}_${arch}_${arm}"
			package_name="${os}-${snap}"
			;;
		"mips"|"mipsle")
			go_build_dir="build_${os}_${arch}_${mips}"
			package_name="${os}-${arch}-${mips}"
			;;
		"mips64"|"mips64le")
			go_build_dir="build_${os}_${arch}_${mips64}"
			package_name="${os}-${arch}-${mips64}"
			;;
		*)
			go_build_dir="build_${os}_${arch}"
			package_name="${os}-${arch}"
			;;
		esac
		go_build_dir="${build_dir}/${go_build_dir}"
		if [ "$os" != "windows" ]; then
			bin_name="xray"
		...
		case "$type" in
		"build")
			build_package "$os" "$arch" "$arm" "$mips" "$mips64"
			;;
		"pack")
			pack_package "$os"
			;;
